...
export interface AppHeaderProps {
  ...
  menuOpen: boolean;
  onMenuModeChange: (mode: 'vertical' | 'icon') => void;
  onMenuOpenChange: (open: boolean) => void;
}

export function AppHeader(props: AppHeaderProps): JSX.Element | null {
  const { sidebarWidth, menuMode, menuOpen, onMenuModeChange, onMenuOpenChange } = props;
  ...
  return (
    <header className={styles['app-header']}>
      <Link className={styles['app-header__logo-container']} to="/">
        ...
        <div
          className={getClassName(styles['app-header__logo-title-wrapper'], 'd-none d-md-block')}
      ...
      <button
        className={getClassName(styles['app-header__button'], 'd-md-none')}
      ...
      <button
        className={getClassName(styles['app-header__button'], 'd-none d-md-inline-flex')}
        aria-label={t(menuMode === 'vertical' ? 'routes.layout.Fold main navigation' : 'routes.layout.Expand main navigation')}
        onClick={() => {
          onMenuModeChange(menuMode === 'vertical' ? 'icon' : 'vertical');
        }}
      >
        {menuMode === 'vertical' ? <MenuFoldOutlined /> : <MenuUnfoldOutlined />}
      </button>
      <button
