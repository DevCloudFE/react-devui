...
// Dont refer to these options, it's out of date
    ...
    for (var i = 0; i < groupCnt; ++i) {
      ...
      for (var j = 0; j < dlen; ++j) {
        if (typeName === 'scatter') {
          ...
          var v = Math.floor(((Math.random() * 600 + 400) * (groupCnt - i)) / groupCnt);
        }
        group.push(v);
      }
      if (typeName === 'radar') {
          ...
          data: (function () {
            ...
            for (var i = 0; i < groupCnt; ++i) {
              ...
              for (var c = 0, clen = Math.ceil(Math.random() * 5); c < clen; ++c) {
                var grand = [];
                var gSum = 0;
                for (var g = 0, glen = Math.ceil(Math.random() * 3); g < glen; ++g) {
                  var v = Math.ceil(Math.random() * 100);
                  grand.push({
                    value: v,
                    name: i + 1 + '-' + (c + 1) + '-' + (g + 1),
                    path: i + 1 + '/' + (i + 1) + '-' + (c + 1) + '/' + (i + 1) + '-' + (c + 1) + '-' + (g + 1),
                  });
                  gSum += v;
                }
                children.push({
