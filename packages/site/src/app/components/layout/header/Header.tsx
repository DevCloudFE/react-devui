...
export function AppHeader(props: { aMenuOpen: boolean; onMenuOpenChange: (open: boolean) => void }) {
  const { aMenuOpen, onMenuOpenChange } = props;

  const { gTheme, gOnThemeChange } = useContextRequired(AppContext);
    ...
    <header className="app-header is-shadow">
      {mediaMatch.includes('md') ? (
        ...
        <button
          ...
          aria-label={t('Open main navigation')}
      ...
      <div className="app-header__button-container">
        ...
        <button
          ...
          onClick={() => {
            gOnThemeChange(gTheme === 'light' ? 'dark' : 'light');
          }}
        >
