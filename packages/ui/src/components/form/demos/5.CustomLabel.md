...
export default function Demo() {
  ...
  return (
    <>
      <DRadioGroup
        dOptions={['required', 'optional', 'hidden'].map((label) => ({
          label: label,
          value: label,
        }))}
        dModel={[label, setLabel]}
        dType="outline"
      />
      <br />
      <DForm dForm={formInstance1} dLayout="vertical" dRequiredType={label}>
        <DFormItem dFormControls={{ username1: 'Please input your username!' }} dLabel="Username">
          {({ username1 }) => <DInput dFormControl={username1} placeholder="Username" />}
        </DFormItem>
        <DFormItem dFormControls={{ username2: 'Please input your username!' }} dLabel="Username">
          {({ username2 }) => <DInput dFormControl={username2} placeholder="Username" />}
        </DFormItem>
      </DForm>
      <br />
